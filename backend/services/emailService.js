...
class EmailService {
  ...
  async initialize() {
    ...
    // Verify connection and validate credentials
    try {
      if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
        console.error('❌ Missing email credentials: SMTP_USER or SMTP_PASS not set');
        console.log('⚠️ Email service initialized without proper credentials');
        return;
      }
      ...
      console.log('✅ Email service initialized and SMTP verified successfully');
    } catch (error) {
      console.error('❌ Email service SMTP verification failed:', error.message);
      if (error.message.includes('Username and Password not accepted')) {
        console.error('🔐 Gmail authentication failed - check SMTP_USER and SMTP_PASS');
      }
    }
  }
