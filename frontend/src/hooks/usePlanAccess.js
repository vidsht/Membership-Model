...
export const usePlanAccess = () => {
  ...
  const checkPlanAccess = () => {
    ...
    // Check validation date first (primary expiry date)
    if (user.validationDate) {
      expiryDate = new Date(user.validationDate);
    } else if (user.planExpiryDate) {
      expiryDate = new Date(user.planExpiryDate);
    } else if (user.subscriptionEndDate) {
      expiryDate = new Date(user.subscriptionEndDate);
    } else if (user.planEndDate) {
      expiryDate = new Date(user.planEndDate);
    }
